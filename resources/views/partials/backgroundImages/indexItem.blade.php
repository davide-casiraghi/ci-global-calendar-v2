        ...
        <div class="px-4 py-4 sm:px-6">
            <div class="mr-2 float-left">
                @if($backgroundImage->hasMedia('background_image'))
                    <img class="h-14" src='{{$backgroundImage->getMedia('background_image')->first()->getUrl('thumb')}}' />
                @endif
            </div>
            <div class="flex items-center justify-between">
                ...
                <div class="ml-2 flex-shrink-0 flex">
                    <p class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full @if($backgroundImage->isPublished())bg-green-100 text-green-800 @else bg-red-100 text-red-800 @endif">
                        {{ucfirst($backgroundImage->publishingStatus())}}
                    </p>
                </div>
            </div>
            <div class="mt-2 sm:flex sm:justify-between">
                ...
                <div class="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                    ...
                    <p>
                        Created on
                        <time datetime="{{$backgroundImage->created_at->format('Y-m-d')}}">
                            {{$backgroundImage->created_at->format('M j, Y')}}
                        </time>
                    </p>
                </div>
            </div>
