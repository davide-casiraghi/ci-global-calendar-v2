<div class="text-gray-600">
    @forelse($events as $event)
        <div class="md:grid md:grid-cols-12 p-1 @if ($loop->iteration % 2 == 0) bg-white @else bg-gray-50 @endif ">
            ...
            <div class="md:col-span-3 px-6 py-4 text-base text-gray-500">
                <a class="textLink" href="{{route('events.show', $event->slug)}}">{{$event->title}}</a>
            </div>
            <div class="md:col-span-3 flex items-center px-6 md:px-0 mb-1 md:mb-0">
                {{-- Teachers --}}
                @if(count($event->teachers))
                    ...
                    <div class="text-sm md:px-4">
                        @foreach ($event->teachers as $key => $teacher)
                            <a class="textLink" href="{{route('teachers.show', $teacher->slug)}}">{{$teacher->full_name}}</a>@if(!$loop->last),@endif
                        @endforeach
                    </div>
                @endif
            </div>
            <div class="md:col-span-2 flex items-center px-6 md:px-0 mb-1 md:mb-0 text-sm">
                {{-- Category --}}
            ...
            <div class="md:col-span-3 flex items-center text-sm px-6 md:px-0 mb-1 md:mb-0">
                {{-- Location --}}
